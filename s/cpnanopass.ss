        ...
        [(hand-coded ,sym)
         (case sym
           ...
           [($native-fiber-allocate-descriptor)
            `(lambda ,(make-named-info-lambda '$native-fiber-allocate-descriptor '(0)) 0 ()
               ,(%seq
                  (set! ,%xp ,(%tc-ref cached-frame))
                  (if ,(%inline eq? ,%xp ,(%constant sfalse))
                      (set! ,%xp ,(%constant-alloc type-closure (constant size-continuation)))
                  ...
                  (set! ,(%mref ,%xp ,(constant continuation-stack-length-disp))
                    (immediate ,(constant scaled-shot-1-shot-flag)))
                  (set! ,(%mref ,%xp ,(constant continuation-stack-clength-disp))
                    (immediate ,(constant scaled-shot-1-shot-flag)))
            ...
            (let ([info (make-named-info-lambda '$native-fiber-switch-entry '(1))])
              (let ([invalid-target
                    ...
                    (if ,(%inline eq? ,%xp ,(%constant sfalse))
                        (nop)
                        ,(%seq
                           (set! ,%ts ,(%tc-ref scheme-stack))
                           (set! ,(%mref ,%xp ,(constant continuation-stack-disp)) ,%ts)
                           (set! ,%ts ,(%tc-ref scheme-stack-size))
                           (set! ,(%mref ,%xp ,(constant continuation-stack-length-disp)) ,%ts)
                           (set! ,%ts ,(%tc-ref scheme-stack))
                           (set! ,%ts ,(%inline - ,%sfp ,%ts))
                           (set! ,(%mref ,%xp ,(constant continuation-stack-clength-disp)) ,%ts)
                           (set! ,%ts ,(%tc-ref stack-link))
                           (set! ,(%mref ,%xp ,(constant continuation-link-disp)) ,%ts)
                           (set! ,%ts ,%ref-ret)
                           (set! ,(%mref ,%xp ,(constant continuation-return-address-disp)) ,%ts)
                           (set! ,%ts ,(%tc-ref winders))
                    ...
                    (if ,(%inline eq? ,%xp ,(%constant sfalse))
                        ,(native-fiber-invariant-failure)
                        (nop))
                    (set! ,%ts ,(%mref ,%xp ,(constant continuation-stack-disp)))
                    (set! ,(%tc-ref scheme-stack) ,%ts)
                    (set! ,%sfp ,%ts)
                    (set! ,%ts ,(%mref ,%xp ,(constant continuation-stack-length-disp)))
                    (set! ,(%tc-ref scheme-stack-size) ,%ts)
                    (set! ,%ts ,(%inline - ,%ts ,(%constant stack-slop)))
                    ,(%set-esp ,(%inline + ,%sfp ,%ts))
                    (set! ,%ts ,(%mref ,%xp ,(constant continuation-stack-clength-disp)))
                    (set! ,%sfp ,(%inline + ,%sfp ,%ts))
                    (set! ,%ts ,(%mref ,%xp ,(constant continuation-link-disp)))
                    (set! ,(%tc-ref stack-link) ,%ts)
                    (set! ,%ts ,(%mref ,%xp ,(constant continuation-winders-disp)))
                    (set! ,(%tc-ref winders) ,%ts)
                    (set! ,%ts ,(%mref ,%xp ,(constant continuation-attachments-disp)))
                    (set! ,(%tc-ref attachments) ,%ts)
                    (jump
                      (literal ,(make-info-literal #f 'library-code
                                  (lookup-libspec $native-fiber-switch-commit-entry)
                                  (constant code-data-disp)))
                      ())))])
              (let ([le
                     `(lambda ,info 0 ()
                ...
                le)))]
           [($native-fiber-switch-commit-entry)
            (let ([info
                   (make-named-info-lambda
                     '$native-fiber-switch-commit-entry '(0))])
              (let ([le
                     `(lambda ,info 0 ()
                        ,(%seq
                           (if ,(%inline eq?
                               ...
                               (nop)
                               ,(native-fiber-invariant-failure))
                           ;; This entry begins only after the target stack and
                           ;; dynamic roots are installed. Isolate the debug
                           ;; hook from the register-tight install and commit
                           ;; blocks so no live VM register crosses the call.
                           (if ,(%inline eq?
                           ...
                           (jump
                             (literal
                               ,(make-info-literal #f 'library-code
                                  (lookup-libspec $native-fiber-switch-finish-entry)
                                  (constant code-data-disp)))
                             ())))])
                (verify-native-fiber-raw-entry
                  '$native-fiber-switch-commit-entry le)
                le))]
           [($native-fiber-switch-finish-entry)
            (let ([info
                   (make-named-info-lambda
                     '$native-fiber-switch-finish-entry '(0))])
              (let ([le
                     `(lambda ,info 0 ()
                        ,(%seq
                           (if ,(%inline eq?
                           ...
                           (set! ,%xp
                             ,(%mref ,%xp
                                ,(constant native-fiber-context-disp)))
                           (set! ,%td ,(%tc-ref current-native-fiber))
                           (set! ,%ts
                             ,(%mref ,%td
                                ,(constant native-fiber-handler-stack-disp)))
                           ...
                           (if ,(%inline eq?
                                  ,(%mref ,%td
                                     ,(constant native-fiber-cache-context-disp))
                                  ,(%constant sfalse))
                               (nop)
                           ...
                           (jump
                             (literal
                               ,(make-info-literal #f 'library-code
                                  (lookup-libspec $native-fiber-switch-publish-entry)
                                  (constant code-data-disp)))
                             ())))])
                (verify-native-fiber-raw-entry
                  '$native-fiber-switch-finish-entry le)
                le))]
           [($native-fiber-switch-publish-entry)
            (let ([info
                   (make-named-info-lambda
                     '$native-fiber-switch-publish-entry '(0))])
              (let ([le
                     `(lambda ,info 0 ()
                        ,(%seq
                           (if ,(%inline eq?
                           ...
                           (jump
                             (literal
                               ,(make-info-literal #f 'library-code
                                  (lookup-libspec $native-fiber-switch-resume-entry)
                                  (constant code-data-disp)))
                             ())))])
                (verify-native-fiber-raw-entry
                  '$native-fiber-switch-publish-entry le)
                le))]
           [($native-fiber-switch-resume-entry)
            (let ([info
                   (make-named-info-lambda
                     '$native-fiber-switch-resume-entry '(0))])
              (let ([le
                     `(lambda ,info 0 ()
                        ,(%seq
                           (if ,(%inline eq?
                           ...
                           (set! ,(%mref ,%xp
                             ...
                             ,(%constant sfalse))
                           (set! ,(%mref ,%xp
                                    ,(constant continuation-stack-length-disp))
                             ,(%constant scaled-shot-1-shot-flag))
                           (set! ,(%mref ,%xp
                                    ,(constant continuation-stack-clength-disp))
                             ,(%constant scaled-shot-1-shot-flag))
                           (set! ,(%mref ,%xp
                                    ,(constant continuation-link-disp))
                             ,(%constant sfalse))
                           (set! ,(%mref ,%xp
                                    ,(constant continuation-winders-disp))
                             ,(%constant snil))
                           (set! ,(%mref ,%xp
                                    ,(constant continuation-attachments-disp))
                             ,(%constant snil))
                           (set! ,%td ,(%tc-ref current-native-fiber))
                           (set! ,(ref-reg %ac1)
                             ,(%mref ,%td
                                ,(constant native-fiber-starter-disp)))
                           (set! ,(%mref ,%td
                                    ,(constant native-fiber-starter-disp))
                             ,(%constant sfalse))
                           (set! ,%ac0
                           ...
                           (set! ,(%tc-ref native-fiber-transition)
                             ,(%constant sfalse))
                           (set! ,(%tc-ref native-fiber-test-active)
                             ,(%constant sfalse))
                           (if ,(%inline eq?
                                  ,(ref-reg %ac1) ,(%constant sfalse))
                               (jump ,%ts (,%ac0))
                               ,(%seq
                                  ...
                                  ,(do-call 0)))))])
                (verify-native-fiber-raw-entry
                  '$native-fiber-switch-resume-entry le)
                le))]
           [(native-fiber-context)
            (let ([info (make-named-info-lambda 'native-fiber-context '(-1))])
              (info-lambda-flags-set! info
                (fxlogor (constant code-flag-continuation) (constant code-flag-system)))
              `(lambda ,info 0 ()
                 ,(%seq
                    (set! ,(ref-reg %cp)
                      (literal ,(make-info-literal #t 'object '$oops
                                  (constant symbol-value-disp))))
                    (set! ,(make-arg-opnd 1) ,(%constant sfalse))
                    (set! ,(make-arg-opnd 2)
                      (literal ,(make-info-literal #f 'object
                                  "attempt to invoke a parked native-fiber context" 0)))
                    ,(do-call 2))))]
           [(dounderflow)
