...
(define check-heap-enabled?
  (foreign-procedure "(cs)check_heap_enabledp" () boolean))
(define enable-check-heap!
  (foreign-procedure "(cs)enable_check_heap" (boolean) void))
...
(define stress-check-native-fiber-invariants!
  (lambda ()
    (when stress-invariants?
      ...
      ;; checkpoint: leaving it enabled would turn every minor collection in
      ;; the stress workload into a full shadow traversal.
      (let ([was-enabled? (check-heap-enabled?)])
        (dynamic-wind
          (lambda () (enable-check-heap! #t))
          (lambda () (collect-rendezvous))
          (lambda () (enable-check-heap! was-enabled?)))))))
...
(define stress-structured-groups
  (lambda (iteration)
    (let ([completed (box 0)])
      (let ([value
             (run-async
               (lambda ()
                 (let ([group (make-task-group)])
                   (do ([i 0 (fx+ i 1)]) ((fx= i 8))
                     (spawn-task
                       (lambda ()
                         (do ([round 0 (fx+ round 1)]) ((fx= round 3))
                           (task-yield)
                           (when (and (fx= i 0) (fx= round 1))
                             (stress-check-native-fiber-invariants!)))
                         (let increment ()
                           (let ([old (atomic-box-ref completed)])
                             (unless (box-cas! completed old (fx+ old 1))
                               (increment)))))
                       'group group
        ...
        (check (eq? value 'root-returned)
          'structured-groups-result iteration value)
        (check (fx= (atomic-box-ref completed) 8)
          'structured-groups-lifetime iteration
          (atomic-box-ref completed))))))
...
(guard (c
         [else
          (finish-watchdog!)
          (display "async stress failed: " (console-error-port))
          (display-condition c (console-error-port))
          (newline (console-error-port))
          (flush-output-port (console-error-port))
          (exit 1)])
  (when stress-invariants?
    (unless (eq? (#3%$async-debug-lock-rank-test) #t)
      (error 'async-stress "async lock-rank checker is not active")))
  (#3%$async-debug-work-deque-model-test 10000)
  (when (threaded?)
    (#3%$async-debug-work-deque-contention-test 20000 8))
  (do ([i 0 (fx+ i 1)]) ((fx= i stress-iterations))
    (when stress-trace?
      (printf "async stress: starting iteration ~s\n" i)
      (flush-output-port (current-output-port)))
    (when (stress-scenario? "cancellation")
      (stress-mark! (list i 'completion-cancellation))
    ...
    (when (stress-scenario? "channel")
      (stress-mark! (list i 'channel-rendezvous))
      (stress-channel-rendezvous i)
      (stress-mark! (list i 'channel-close-race))
      (stress-channel-close-race i))
    (when (stress-scenario? "mutex")
      (stress-mark! (list i 'async-mutex))
      (stress-async-mutex i))
    (when (stress-scenario? "groups")
      (stress-mark! (list i 'structured-groups))
      (stress-structured-groups i))
    (when (stress-scenario? "preemption")
      (stress-mark! (list i 'native-preemption))
      (stress-native-preemption i))
    (when (stress-scenario? "sync")
      (stress-mark! (list i 'sync-primitives))
      (stress-sync-primitives i))
    (when (and stress-io? (stress-scenario? "io"))
      (stress-mark! (list i 'file-owner-routing))
